...
import argparse
import random
from measure_SICE import measure#zj
from eval_SICE import eval#zj
import shutil
...
parser.add_argument('--data_train', type=str, default='../PairLIE/PairLIE-training-dataset/')#将dataset改成PairLIE
...
last_path=""#zj
last_psnr,last_ssim,last_lpips=0,0,0#zj
for epoch in range(opt.start_iter, opt.nEpochs + 1):
    train()#原来是train()
    scheduler.step()
    now_path=checkpoint(epoch)
    eval(now_path)
    avg_psnr, avg_ssim, avg_lpips=measure()
    if epoch==1 :#zj
        last_psnr,last_ssim,last_lpips=avg_psnr, avg_ssim, avg_lpips
        last_path=now_path
    else:
        if avg_psnr>last_psnr or (avg_psnr==last_psnr and avg_ssim>last_ssim) or (avg_psnr==last_psnr and avg_ssim==last_ssim and avg_lpips<last_lpips):
            try:
                os.remove(last_path)
                last_path=now_path
                last_psnr, last_ssim, last_lpips = avg_psnr, avg_ssim, avg_lpips
