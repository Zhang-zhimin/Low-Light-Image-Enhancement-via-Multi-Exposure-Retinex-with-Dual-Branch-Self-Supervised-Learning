...
def metrics(im_dir, label_dir):
    ...
    for item in sorted(glob.glob(im_dir)):
        ...
        name = item.split('\\')[-1]#原来是item.split('/'),但是item中路径是../PairLIE/PairLIE-our-results/LOL-test/R\1.png
...
# if __name__ == '__main__':  此行是源代码上的，我要改成从main中调用，所以注释掉
def measure():
    im_dir = '../PairLIE/PairLIE-our-results/LOL-test/I/*.png'#'results/LOL/I/*.png'改为'../PairLIE/PairLIE-our-results/LOL-test/I/'
    label_dir = '../PairLIE/PairLIE-testing-dataset/LOL-test/reference/'#'../dataset/LIE/LOL-test/reference/'改为../PairLIE/PairLIE-testing-dataset/LOL-test/reference/
